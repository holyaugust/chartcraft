import { BarChart2, BarChart3, Brain, FileText, GitBranch, LayoutGrid, Palette } from 'lucide-react'

export type WorkspaceId = 'chart' | 'document' | 'flowchart' | 'mindmap' | 'smartgraphic' | 'pptbeautify'
    ...
    <header className="app-header">
      <div className="header-left">
        ...
        <nav className="header-workspace-nav" aria-label="工作区切换">
          ...
          <button
            type="button"
            className={`workspace-nav-btn${workspace === 'pptbeautify' ? ' active' : ''}`}
            title="PPT 美化"
            aria-label="PPT 美化"
            aria-current={workspace === 'pptbeautify' ? 'page' : undefined}
            onClick={() => onWorkspaceChange('pptbeautify')}
          >
